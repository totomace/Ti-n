...
@Composable
fun DashboardCards(entries: List<WorkEntry>) {
    ...
    val totalIncome by animateIntAsState(
        targetValue = thisMonthEntries.sumOf { it.salary.toInt() },
        animationSpec = tween(600),
        label = "income"
    )
    
    val pendingCount by animateIntAsState(
        targetValue = thisMonthEntries.count { it.paidAmount < it.salary },
        animationSpec = tween(600),
        label = "pending"
    )
    
    val completedCount by animateIntAsState(
        targetValue = thisMonthEntries.count { it.paidAmount >= it.salary },
        animationSpec = tween(600),
        label = "completed"
    )
    ...
    Row(
        modifier = Modifier
            ...
            .graphicsLayer {
                ...
                scaleX = cardsScale.value
                scaleY = cardsScale.value
            },
        horizontalArrangement = Arrangement.spacedBy(8.dp)
    ) {
        // Total Income Card
        Card(
            ...
            Column(
                ...
                Text(
                    NumberFormat.getNumberInstance(Locale("vi", "VN")).format(totalIncome),
                    fontSize = 16.sp,
                    fontWeight = FontWeight.Bold,
                    color = Color.White,
                    maxLines = 1
